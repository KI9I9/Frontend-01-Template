...
class Request {
  ...
  constructor(options) {
    ...
    if (this.headers["Content-Type"] === enumContentType.json) {
      this.bodyText = JSON.stringify(this.body);
    } else if (this.headers["Content-Type"] === enumContentType.form) {
  ...
  receiverChar(char) {
    ...
    if (this.current === this.READING_LENGTH_FIRST_CHAR) { // Length的第一个字符是单独一个状态
      ...
      if (char === '\r') {
        ...
        this.chunkLength = this.chunkLength * 16 + parseInt(char, 16);
      }
...
void async function () {
  ...
  console.log("response", JSON.stringify(dom, null, "    "));
}();
